    ...
    def run(self, plan: Dict[str, Any], query: str, attempt: int = 1) -> Tuple[List[Dict[str, Any]], Dict[str, Any], str]:
        ...
        if self.use_llm:
            ...
            1. `companies` (
                ...
                name TEXT NOT NULL,
                domain TEXT,
                industry TEXT (values: 'AI SaaS', 'Fintech', 'Data Infrastructure', 'Cybersecurity', 'E-commerce', 'HR SaaS', etc.),
                description TEXT,
                location TEXT (EXACT values stored: 'US', 'Europe', 'India' — use LOWER(location) LIKE '%india%' for India, LIKE '%us%' for US, LIKE '%europe%' for Europe. NEVER use exact equality for location!),
                size TEXT (values: '50-100', '100-250', '20-50', '250-500', '1000-5000', '2000-5000'),
                funding_stage TEXT (values: 'Seed', 'Series A', 'Series B', 'Series C', 'Series E', 'Series F'),
                funding_amount TEXT (values like '$12M', '$28M', '$741M', '$1B'),
                growth_rate TEXT (values: '150% YoY', '80% YoY', '300% YoY', '45% YoY', '120% YoY', '200% YoY', '250% YoY', '90% YoY' - DO NOT filter growth_rate using string matches like '%high%' or '%growth%'. Always leave this column unfiltered in SQL and let the GTM Strategy Agent score growth rate instead!),
                hiring_status TEXT (values: 'Aggressive hiring', 'Moderate hiring', 'Frozen' - DO NOT filter hiring_status using string matches like '%aggressively%'. If filtering for hiring status, use LOWER(hiring_status) LIKE '%aggressive%' or LOWER(hiring_status) LIKE '%moderate%'),
